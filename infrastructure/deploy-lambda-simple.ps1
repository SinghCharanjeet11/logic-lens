...
"@
        ...
        $trustPolicy | Out-File -FilePath $trustPolicyPath -Encoding UTF8 -NoNewline
...
function Deploy-Lambda {
    ...
    try {
        $result = aws lambda get-function --function-name $FunctionName --region $REGION 2>&1
        if ($LASTEXITCODE -eq 0) {
            $functionExists = $true
        }
    } catch {}
...
Write-Host "1. Run setup-api-gateway-simple.ps1 to create API endpoints" -ForegroundColor White
